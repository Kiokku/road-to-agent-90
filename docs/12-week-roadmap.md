# 12 个 Week 纵切路线：Web Creation Agent Workbench

顺序：Week 01 → Week 12；当前 Week 完成并验证后才进入下一个 Week
预算：每个 Week 约 10 小时，共约 120 小时
完成定义：经过测试、能够演示，并能在 5 分钟内闭卷解释机制、失败路径和技术取舍。

每个 Week 只交付一个纵切。先完成资料获取、检索练习和小实验，再接入 `projects/web-creation-agent-workbench/`，运行测试并留下演示证据。
...
- 约 4 小时：读一手资料、闭卷检索、小实验、准备实现。
- 约 6 小时：接入主作品、运行测试、录制或保存演示证据。
- Week 结束前：完成 5 分钟闭卷讲解；不能解释清楚则该纵切仍为进行中。
